...
def candidate_register(request):
    if request.method=='POST':
        ...
        jobtypes=jobtypes,skills=skills,experience=experience,comments=comments,phoneno=phoneno)
...
def employer_register(request):
    if request.method=='POST':
        ...
        jobtypes=jobtypes,skills=skills,work_hours=work_hours,comments=comments,contactno=contactno,no_of_vacancies=no_of_vacancies,company_name=company_name)

        if employer_object is not None:
            employer_object.save()
            open_jobs=OpenJobs(email=email,company_email=company_email,name=name,address=address,state=state,pincode=pincode,salary=salary,
            jobtypes=jobtypes,skills=skills,work_hours=work_hours,comments=comments,contactno=contactno,no_of_vacancies=no_of_vacancies,company_name=company_name,isVerified=False) 
            open_jobs.save()
